...
class PreferenceRepositoryImplTest : BaseAndroidTest() {
    ...
    fun setup() {
        testDataStore = PreferenceDataStoreFactory.create(
            ...
            produceFile =
            { appContext.preferencesDataStoreFile(TEST_DATASTORE_NAME) }
        )
    ...
    @Test
    fun testSaveGetIntSuccess() = testCoroutineScope.runTest {
        val mainValue = 111
        preferenceRepository.save(PreferenceKey.UserAge, mainValue)
        val result = preferenceRepository.get(PreferenceKey.UserAge, 111)
        result shouldBeEqualTo mainValue
    }
